	        ...
	        if( isset( $name ) AND ( $name != '' OR $name != NULL) ){
	            ...
	            if( is_array($check) AND sizeof($check)>0 ){
	                ...
	                if( file_exists($filename)){
	                	...
	                	$filen = basename($file_path . $name);
	            		$filep = str_replace($filen, '', $file_path . $name);
	                    trigger_error( 'No file with name '.$filen );
				    }
			    }
	            else{
				    $filen = basename($file_path . $name);
	            	$filep = str_replace($filen, '', $file_path . $name);
				    trigger_error( 'No file with name <b>'.$filen.'</b> in <b>'.$filep.'</b>' ,E_USER_ERROR);
			    }
